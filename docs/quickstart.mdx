...
For production deployments that rely on OAuth/SSO or other rotating
credentials, you can also configure the SDKs to send an
`Authorization: Bearer` header via a header provider, on top of the
required `x-api-key`. This keeps token refresh logic in one place while
all client calls continue to use the standard `connect` API.
