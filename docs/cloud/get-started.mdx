...
<Info>
For Enterprise and SSO/IDP-backed deployments where your access token rotates
frequently, you can combine the required `x-api-key` header with an
`Authorization: Bearer` token managed by the SDKs. See the Python and
TypeScript examples below for a typical pattern.
</Info>
