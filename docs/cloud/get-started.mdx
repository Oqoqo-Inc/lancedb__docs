...
By default, most examples use `api_key`-based authentication. For production and enterprise deployments, you can also use OAuth/Bearer tokens via the SDK header providers.
...
# Connect to LanceDB Cloud/Enterprise with API key
db = lancedb.connect(
    uri="db://your-database-uri",
    api_key="your-api-key",
    region="us-east-1",
    host_override=os.environ.get("LANCEDB_HOST_OVERRIDE"),  # Enterprise only
)
```
...
const hostOverride = process.env.LANCEDB_HOST_OVERRIDE; // Enterprise only
...
### Optional: OAuth/Bearer authentication via header providers

For environments where you obtain short-lived access tokens from an IdP or OAuth server, you can configure the SDKs to send `Authorization: Bearer` headers automatically.

First, import the OAuth header providers and configure token fetchers in your application code. The examples below are also available as snippets from `/docs/snippets/oauth-auth.mdx`.
...
# Connect using OAuth/Bearer auth
db = lancedb.connect(
    uri="db://your-database-uri",
    region="us-east-1",
