...
If you use **short-lived or rotating cloud credentials**, you can also attach a `StorageOptionsProvider` when creating or opening tables. See [Dynamic / expiring credentials with StorageOptionsProvider](/storage/configuration#dynamic--expiring-credentials-with-storageoptionsprovider) for details.
