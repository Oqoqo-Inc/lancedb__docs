...
> **Important:** `rerank_hybrid` **must** return a `pyarrow.Table` that contains a `_relevance_score` column.
> LanceDB will validate this at runtime and raise an error if the column is missing.
...
class MyReranker(Reranker):
    ...
    def rerank_hybrid(self, query: str, vector_results: pa.Table, fts_results: pa.Table):
        ...
        # Compute your relevance scores here and attach them
        relevance_scores = ...  # 1D array-like of float scores
        combined_result = combined_result.append_column(
            "_relevance_score", pa.array(relevance_scores, type=pa.float32())
        )

        # Return the combined results with the required _relevance_score column
