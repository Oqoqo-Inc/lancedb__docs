...
import {
    ...
    PyStorageOptionsProviderBasic,
    PyStorageOptionsProviderNamespace,
    PyStorageOptionsProviderWithExpiration,
    PyStorageS3Ddb,
    PyStorageS3Express,
    PyStorageS3Minio,
...
For production environments with temporary or time-limited credentials, use a `StorageOptionsProvider` to automatically refresh credentials before they expire. This is essential for long-running processes using IAM roles, STS sessions, or namespace-managed credentials.

<Info title="Provider types">
- **Static credentials**: Use `storage_options` dictionary (shown above)
- **Dynamic credentials**: Use `storage_options_provider` parameter with a custom provider
</Info>

### Python: Custom provider

Implement `StorageOptionsProvider` and pass it to `create_table()` or `open_table()`:

<CodeGroup>
    <CodeBlock language="Python" title="Custom storage options provider" icon="python">
    {PyStorageOptionsProviderBasic}
    </CodeBlock>
</CodeGroup>

### Python: Provider with expiration

Include `expires_at_millis` to enable automatic refresh:

<CodeGroup>
    <CodeBlock language="Python" title="Provider with expiration" icon="python">
    {PyStorageOptionsProviderWithExpiration}
    </CodeBlock>
</CodeGroup>

### Python: Namespace storage options provider

Use `LanceNamespaceStorageOptionsProvider` to fetch time-limited credentials from the namespace service:

<CodeGroup>
    <CodeBlock language="Python" title="Namespace storage options provider" icon="python">
    {PyStorageOptionsProviderNamespace}
    </CodeBlock>
</CodeGroup>

### Rust: Storage options provider

Implement the `StorageOptionsProvider` trait and pass it to the connection or read params:
...
For more details, see the [dynamic credentials guide](/storage/dynamic-credentials).
