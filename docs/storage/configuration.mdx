...
### Dynamic storage options for temporary credentials

In production, you often authenticate with short-lived credentials from an identity provider or metadata service instead of long-lived access keys.

LanceDB supports this with a **storage options provider** that refreshes credentials on demand. Instead of passing a static `storage_options` dictionary, you pass an object with a `fetch_storage_options()` method.

When LanceDB needs to perform I/O, it calls `fetch_storage_options()` and uses the returned dictionary, so you can rotate or refresh credentials transparently.
...
class MyStorageOptionsProvider:
    def fetch_storage_options(self) -> Dict[str, Any]:
        # Replace this with your cloud-specific refresh logic
        # (for example, call your IDP or metadata service to get
        # a fresh set of time-limited credentials).
        return {
...
table = db.create_table("my_table", data, storage_options_provider=provider)
existing = db.open_table("my_table", storage_options_provider=provider)
```

#### Rust

In Rust, pass a storage options provider when opening or creating a table. The provider is called whenever LanceDB needs to refresh storage credentials.

```rust
use lancedb::connect;
// use your own type that implements the appropriate trait for
// fetching storage options from your credential source.

# fn main() -> lancedb::Result<()> {
let db = connect("s3://bucket/path")
    .storage_options_provider(/* your provider here */)
    .execute()?;
...
For a concrete implementation, see the built-in `LanceNamespaceStorageOptionsProvider`, which fetches time-limited credentials from the namespace service (defined in the LanceDB codebase under `namespace.py`).
