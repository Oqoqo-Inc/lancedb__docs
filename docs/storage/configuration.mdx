...
In production environments that use short‑lived or rotating cloud credentials (for example STS, federation, or workload identity), pass a `storage_options_provider` instead of static `storage_options`.

The provider is responsible for returning fresh storage options each time LanceDB needs to access the table.
...
class MyStorageOptionsProvider:
    """Fetch fresh, time‑limited storage options."""

    def fetch_storage_options(self) -> Dict[str, str]:
        # Call your internal metadata or secrets service here
        # and return a flat dict of string keys and values.
        # The dict can include an "expires_at_millis" key that
        # LanceDB uses to decide when to refresh credentials.
        return {
            "aws_access_key_id": "...",
            "aws_secret_access_key": "...",
            "aws_session_token": "...",
            # Optional: epoch millis when these options expire
...
# Table‑level providers are also supported
# (overrides the connection‑level provider for this table only)
...
LanceDB will call `fetch_storage_options()` whenever the current options are about to expire. If you include an `expires_at_millis` key (milliseconds since Unix epoch) in the returned dict, LanceDB uses that value to schedule refreshes.

The same pattern is available in the Rust API via the `storage_options_provider` parameter on `create_table` and `open_table`.
