...
For production environments with temporary or expiring credentials, use `StorageOptionsProvider` for automatic credential refresh. See [Dynamic Credentials](/storage/dynamic-credentials) for details.
